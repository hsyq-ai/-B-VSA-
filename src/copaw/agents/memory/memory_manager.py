...
class MemoryManager(ReMeLight):
    ...
    def set_current_profile_id(self, profile_id: str) -> None:
        """
        设置当前请求的 profile_id，用于记忆检索隔离
        
        Args:
            profile_id: 用户档案 ID
        """
        old_profile_id = self._current_profile_id
        self._current_profile_id = profile_id
        logger.info(
            "[MemoryManager] Profile ID changed: %s -> %s",
            old_profile_id or "(none)",
            profile_id,
        )
