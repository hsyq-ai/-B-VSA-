...
- 生成给用户下载的文件时，默认要求 **兼容 Ubuntu 生成、Mac/Windows 打开**。
- 如果内容包含中文，**不要** 使用 reportlab 默认字体直接输出 PDF，否则很容易乱码。
- 中文文档优先策略：
  1. 先生成 `.docx`
  2. 再使用 `soffice --headless --convert-to pdf` 导出 PDF
- 只要最终目标包含 `PDF`，就必须先产出 `.docx`，再自动导出 `.pdf`，不要跳过中间的 `.docx`。
- 如果必须直接用 Python 生成 PDF，必须显式注册并嵌入中文字体，例如：
  - `Noto Sans CJK SC`
  - `WenQuanYi Zen Hei`
  - `WenQuanYi Micro Hei`
- 一个回复里如果生成了多个文件，要逐个明确列出文件名，并为每个文件都提供下载入口，不要只给第一个文件。
