...
def _normalize_remote_url(url: str) -> str:
    ...
    if parsed.scheme or parsed.netloc:
        if parsed.path.startswith("/api/chat-files/"):
            base = _resolve_api_base_url()
            rewritten = urllib.parse.urljoin(base, parsed.path)
            if parsed.query:
...
def _extract_chat_file_id(parsed: urllib.parse.ParseResult) -> Optional[str]:
    if not parsed.path.startswith("/api/chat-files/"):
        return None
    parts = [p for p in parsed.path.split("/") if p]
    # expected: ["api", "chat-files", "{file_id}", "download"]
    if len(parts) < 3:
        return None
    return parts[2]


def _resolve_chat_file_local(url: str) -> Optional[str]:
    parsed = urllib.parse.urlparse(url)
    file_id = _extract_chat_file_id(parsed)
    if not file_id:
        return None
    try:
        from ...app.chat_file_store import ChatFileStore

        store = ChatFileStore(WORKING_DIR / "chat_files.db")
        meta = store.get_file(str(file_id))
        if not meta:
            return None
...
async def download_file_from_url(
    ...
    try:
        ...
        if local is not None:
            return local
        local_chat_file = _resolve_chat_file_local(url)
        if local_chat_file is not None:
            return local_chat_file

        remote_url = _normalize_remote_url(url)
        remote_parsed = urllib.parse.urlparse(remote_url)
        download_path = Path(download_dir)
        download_path.mkdir(parents=True, exist_ok=True)
        if not filename:
            url_filename = os.path.basename(remote_parsed.path)
            filename = (
                url_filename
                if url_filename
                else f"file_{hashlib.md5(remote_url.encode()).hexdigest()}"
            )
            if filename == "download":
                file_id = _extract_chat_file_id(remote_parsed)
                if file_id:
                    filename = file_id
        local_file_path = download_path / filename
        _download_remote_to_path(remote_url, local_file_path)
        ...
        if local_file_path.suffix == ".file":
            real_suffix = _guess_suffix_from_url_headers(remote_url)
            if not real_suffix:
