...
class AgentsRunningConfig(BaseModel):
    ...
    memory_min_recent_messages: int = Field(
        default=6,
        ge=1,
        le=30,
        description="Minimum recent messages to keep during hard truncation",
    )

    memory_compact_max_rounds: int = Field(
        default=2,
        ge=1,
        le=5,
        description="Maximum rounds of summary compaction in one pre-reasoning pass",
    )

    memory_hard_truncate_enabled: bool = Field(
        default=True,
        description="Whether to enable hard truncation fallback when compaction is insufficient",
    )

    memory_fallback_summary_chars: int = Field(
        default=1200,
        ge=200,
        le=8000,
        description="Max characters for fallback truncation summary",
    )
