...
class Provider(ProviderInfo, ABC):
    ...
    def derive_model_source(self) -> str:
        total_builtin = len(self.models)
        total_extra = len(self.extra_models)
        if self.is_local:
            return "embedded"
        if self.is_custom and total_builtin == 0 and total_extra > 0:
            return "custom-only"
        if total_builtin > 0 and total_extra > 0:
            return "mixed"
        if total_extra > 0:
            return "user-extended"
        return "built-in"

    def derive_risk_level(self) -> str:
        if self.is_local:
            return "low"
        if self.is_custom:
            return "high"
        if self.require_api_key and not self.freeze_url:
            return "medium"
        return "low"

    def derive_health(self) -> str:
        total_models = len(self.models) + len(self.extra_models)
        configured = (
            self.is_local
            or (self.is_custom and bool(self.base_url))
            or (not self.require_api_key)
            or bool(self.api_key)
        )
        if configured and total_models > 0:
            return "ready"
        if configured:
            return "degraded"
        return "unconfigured"

    def derive_policy_tags(self) -> List[str]:
        tags: List[str] = []
        tags.append("local-runtime" if self.is_local else "remote-runtime")
        tags.append("custom-provider" if self.is_custom else "built-in-provider")
        tags.append("frozen-url" if self.freeze_url else "editable-url")
        tags.append("requires-api-key" if self.require_api_key else "keyless-allowed")
        if self.models:
            tags.append("built-in-model-catalog")
        if self.extra_models:
            tags.append("user-extended-models")
        return tags
