        ...
        if os.path.exists(session_save_path):
            async with aiofiles.open(
                ...
                if not content.strip():
                    logger.warning(
                        "Session state file is empty, fallback to empty dict: %s",
                        session_save_path,
                    )
                    return {}
                try:
                    ...
                    broken_path = f"{session_save_path}.broken.{int(time.time())}"
                    try:
                        os.replace(session_save_path, broken_path)
                    except OSError:
                        broken_path = session_save_path
                    logger.warning(
                        "Session state file is invalid JSON, reset to empty dict: %s backup=%s",
                        session_save_path,
                        broken_path,
                    )
                    return {}
