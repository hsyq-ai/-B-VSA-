    ...
    >
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
          <Tag color={riskColorMap[String(provider.risk_level || "medium")] || "default"}>
            风险：{provider.risk_level || "medium"}
          </Tag>
          <Tag color={healthColorMap[String(provider.health || "unconfigured")] || "default"}>
            健康：{provider.health || "unconfigured"}
          </Tag>
          <Tag color="geekblue">来源：{provider.model_source || "built-in"}</Tag>
          {(provider.policy_tags || []).slice(0, 3).map((tag) => (
            <Tag key={tag}>{tag}</Tag>
          ))}
        </div>
